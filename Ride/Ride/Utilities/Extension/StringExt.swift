...
extension String {
    var dropCountryCode: String {
        get {
            var updatedText = self
            for code in Countries.getAllCountriesCode() {
                let prefix = code
                if self.hasPrefix(prefix) {
                    updatedText = String(self.dropFirst(prefix.count).trimmingCharacters(in: .whitespacesAndNewlines))
                    break
                }                
            }
            return updatedText
