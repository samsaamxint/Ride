...
@IBDesignable open class PinCodeTextField: UIView {
    ...
    @IBInspectable public var characterBackgroundCornerRadius: CGFloat = 8
...
//    public var textContentType: UITextContentType! = nil
    ...
    func setError() {
        for background in backgrounds {
            background.addDoubleBorder(with: .clear, and: .errorRed, cornerRadius: 12)
        }
    }
    ...
    private func createLabel() -> UITextField {
        let label = UITextField(frame: CGRect())
        label.font = font
        label.backgroundColor = UIColor.clear
        label.textAlignment = .center
        label.textContentType = .oneTimeCode
        return label
    }
    ...
    private func layoutCharactersAndPlaceholders() {
        ...
        for background in backgrounds {
            let index = backgrounds.firstIndex(of: background) ?? 0
            if (!highlightInputUnderline || !isInput(index)) && isPlaceholder(index) {
                backgrounds[index].addPinCodeDoubleBorder(with: .lightGray, and: .clear, cornerRadius: 12)
            }
            else{
                backgrounds[index].addPinCodeDoubleBorder(with: .black, and: .primaryGreen, cornerRadius: 12)
            }
        }
    }
