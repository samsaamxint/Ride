    ...
    class Autocomplete {
        ...
        class func initWithData(_ data:[String:Any]) -> Autocomplete {
            ...
            if let pId = data["place_id"] as? String,let address = data["description"] as? String {
                ...
                if let structured_formatting = data["structured_formatting"] as? [String: Any] {
                    if let main_text = structured_formatting["main_text"] as? String {
                        object.mainText = main_text
                    }
                    if let secondary_text = structured_formatting["secondary_text"] as? String {
                        object.subText = secondary_text
                    }
                }
            }
...
class GoogleApi : NSObject {
    ...
    func getUrl(_  usedFor:UsedFor,input:String) -> String {
        ...
        case .autocomplete:
            ...
            #if Dev
            let searchURL = String(format: "%@?input=%@&types=%@&language=%@&components=&key=%@&location=%@&radius=%@",GoogleUrl.autocomplete.rawValue , updatedInput, "geocode|establishment", (Commons.isArabicLanguage() ? "ar" : "en"),googleApiKey, "\(LocationManager.shared.lastLocation?.coordinate.latitude ?? 0),\(LocationManager.shared.lastLocation?.coordinate.longitude ?? 0)", "1500")
//            let searchURL = String(format: "%@?input=%@&types=%@&language=%@&components=country:%@&key=%@&location=%@&radius=%@",GoogleUrl.autocomplete.rawValue , updatedInput, "geocode|establishment", (Commons.isArabicLanguage() ? "ar" : "en"),"sa",googleApiKey, "\(LocationManager.shared.lastLocation?.coordinate.latitude ?? 0),\(LocationManager.shared.lastLocation?.coordinate.longitude ?? 0)", "1500")
        ...
        case .placeInformation:
            return String(format: "%@?placeid=%@&language=%@&key=%@",GoogleUrl.placeInfo.rawValue, input, (Commons.isArabicLanguage() ? "ar" : "en"),googleApiKey)
        case .reverseGeo:
